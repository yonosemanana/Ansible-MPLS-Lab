...
def ios_vrf_rt(text_config):
    pattern_vrf = "vrf\s+definition\s+(?P<vrf>\S+)"
    regex_vrf = re.compile(pattern_vrf)
    pattern_rt_import = "route-target\s+import\s+(?P<rt_import>\S+)"
    regex_rt_import = re.compile(pattern_rt_import)
    pattern_rt_export = "route-target\s+export\s+(?P<rt_export>\S+)"
    regex_rt_export = re.compile(pattern_rt_export)

    text_lines = text_config.split("\n")
    vrfs = {} # Dictionary for results. Keys - VRF names, values: dictionaries with keys "route_import" and "route_export" and values - RT communities
